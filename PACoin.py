...
class PACoin:
    ...
    def __init__(self, server:grpc.Server, bind:str, port:int, first_peer:str, db:sqlite3.Connection, timeout:float, peer_num:int):
        self.to_exit = False
        self.server = server
    ...
    def update_latency(self, peer, latency):
        ...
        cursor.execute("UPDATE peers SET latency = ? WHERE host = ?", (latency, peer))
        if cursor.rowcount == 0:
            cursor.execute("INSERT INTO peers (host, latency) VALUES (?, ?)", (peer, latency))
...
#        print(cursor.fetchone())
    ...
    def init_latency(self, peer):
        ...
        cursor.execute("SELECT * FROM peers WHERE host = ?", (peer, ))
        if cursor.rowcount == 0:
            cursor.execute("INSERT INTO peers (host, latency) VALUES (?, ?)", (peer, self.timeout))
#        print(cursor.fetchone())
        cursor.close()
        self.db_mutex.release()

    def list_peers(self, num):
        self.db_mutex.acquire()
        cursor = self.db.cursor()
        cursor.execute("SELECT host FROM peers ORDER BY latency ASC LIMIT ?", (num, ))
#        print(cursor.fetchone())
    ...
    def shrink_peers(self, num, if_print=False):
        self.db_mutex.acquire()
        cursor = self.db.cursor()
        cursor.execute("DELETE FROM peers WHERE host NOT IN (SELECT host FROM peers ORDER BY latency ASC LIMIT ?)", (num, ))
        if if_print:
    ...
    def start(self):
        cursor = self.db.cursor()
        cursor.execute("CREATE TABLE IF NOT EXISTS peers (host VARCHAR(128) PRIMARY KEY, latency REAL)")
        cursor.close()
        self.init_latency(self.first_peer)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--peer', type=str, default="127.0.0.1:23333", help='First peer to connect')
    parser.add_argument('--db', type=str, default="PACoin.db", help='Database file name')
    parser.add_argument('--timeout', type=float, default=2000.0, help='Threshold for timeout')
    parser.add_argument('--peer_num', type=int, default=5, help='Maximum numbers of peers')
    args = parser.parse_args()
    logging.basicConfig()

    pacoin = PACoin(server=grpc.server(concurrent.futures.ThreadPoolExecutor(max_workers=4)), bind=args.bind, port=args.port, first_peer=args.peer, db=sqlite3.connect(args.db, check_same_thread = False), timeout=args.timeout, peer_num=args.peer_num)
    pacoin.start()
